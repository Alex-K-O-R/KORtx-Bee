                ...
                <div class="one-tr">
                    ...
                    <p>
                        ...
                        <ul>
                            ...
                            <li>nodes - дополнительные модули-модификаторы отображения на основе php, js, css, например: меню, редакторы. </li>
                        </ul>
                        Введено также разграничение на системные и пользовательские (расширенные, кастомизированные) классы по директориям: ./original и ./expansion соответственно.
                    </p>
                    <p>
                    ...
                    <h4>Страницы, шаблоны и темы</h4>
                    <p>
                        Всё, что относится к отображению (т.е. всё, что предназначено для клиентской стороны) располагается в разделе display, который содержит вложенные каталоги для простого и понятного разделения на направления: css, img, js, ...
                    </p>
                    <p>
                        Особое внимание следует уделить перечисленным папкам:
                        <ul>
                        <li>general - там описываются темы (./themes/), а также общие шаблоны, например, заголовка и подвала;</li>
                        <li>pages - директория, содержащая в себе страницы/виды, использующиеся в контроллерах для формирования результатов отображения с выбранной темой оформления.</li>
                        </ul>
                    </p>
                    <p>
                        Темы представляют собой предопределённые наборы файлов стилей и сценариев. Эти файлы распространяются на все сегменты сайта, использующие конкретную тему. Кроме того, стили и сценарии из конкретной темы имеют строгий приоритет и порядок загрузки в соответствии с файлом-описателем. Описание тем происходит внутри трейта \app\pages\PageThemes, с помощью метода LoadThemes(...).
                    </p>
                    <p>
                    ...
                    <h4>Ноды</h4>
                    <p>
                        На страницах сайтов часто встречаются элементы отображения, использующиеся в различных сегментах, и не участвующие при этом в работе ядра и модификации объектов и данных.
                        Например, это могут быть функциональные элементы, вроде слайдеров картинок, меню разделов, или визуальных полей форм для редактирования.
                    </p>
                    <p>
                        Для описания подобных элементов KORtx Bee предлагает механизм нодов.
                    </p>
                    <p>
                        Ноды полагается размещать вне файлов ядра и интерфейса, в отдельной директории /nodes/expansion/. Внутри указанной папки допускается любое логическое разделение на субдиректории.
                    </p>
                    <p>
                        Произвольный нод представляет собой оконечную директорию с некоторым названием и вложенным одноименным файлом *.php. Например, /nodes/expansion/others/<b>Warning</b>/<b><i>Warning</i></b>.php.
                        Внутри этой папки наряду с исполняемыми файлами *.php надлежит размещать также файлы стилей *.css и файлы клиенстких сценариев javascript (*.js), а также графические ресурсы.
                    </p>
                    <p>
                        KORtx Bee автоматически загружает все ноды с корректным описанием путей после загрузки файлов ядра, но до рендеринга интерфейса. Соответственно, файлы стилей и сценариев javascript попадают в конец описания внутри заголовка страницы, т.е. после стилей и сценариев визуальных тем сайта.
                    </p>
                    <p>
                        KORtx Bee предлагает два базовых статических класса для описания ноды: app\nodes\Node и app\nodes\NodeOptions. Использование этих классов позволит централизовать визуализацию нодов для кэширования результатов отображения любым удобным способом (см. функцию \app\nodes\Node::Draw(...)).
                        Впрочем, это не обязательное требование &mdash; допустимо использовать произвольные классы, следуя правилам наименования и размещения файлов.
                    </p>
                    ...
                    <p>
                        ...
                        <ul>
                            ...
                            <li>utilities\ - в зависимости от расположения директории, либо общие вспомогательные классы
                                (в т.ч. функции ядра), либо сторонние библиотеки которые могут пригодится где угодно;</li>
                            <li>nodes\ - в ней сосредоточены элементы "распространенной" природы на основе php, js, css, которые могут быть не привязаны к конкретному окружению и в основном работают "на отображение".</li>
                        </ul>
                    </p>
                    <p>
                        Для упорядочивания субдиректории имеют разграничение на original и expansion:
                        ./original - файлы основы фреймворка, которые крайне нежелательно менять, применяются в том числе для обратной совместимости;
                        ./expansion - любые пользовательские расширения файлов KORtx Bee и любые дополнительные классы - лучшее место, чтобы творить волшебство!
                    </p>
